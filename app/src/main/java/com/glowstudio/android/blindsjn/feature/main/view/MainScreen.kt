...
/**
 * 앱의 메인 화면을 구성하는 컴포저블 함수로, 상단바, 하단 네비게이션 바, 내부 네비게이션 콘텐츠를 통합하여 관리합니다.
 *
 * 상단바는 TopBarViewModel의 상태에 따라 동적으로 메인 또는 상세 형태로 전환되며, 하단에는 네비게이션 바가 고정되어 있습니다.
 * 내부 콘텐츠 영역은 NavHost를 통해 다양한 화면(홈, 게시판, 인기글, 메시지, 프로필, 상세/작성/일정/뉴스 등)으로 네비게이션이 가능합니다.
 *
 * @param topBarViewModel 상단바 상태를 관리하는 ViewModel. 기본값은 viewModel()로 제공됩니다.
 * @param navigationViewModel 하단 네비게이션 상태를 관리하는 ViewModel. 기본값은 viewModel()로 제공됩니다.
 */
@Preview
@Composable
fun MainScreen(
